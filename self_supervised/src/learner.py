...
class Learner(GetAttr):
    ...
    def __init__(self, args, dls, model, 
                        ...
                        ft=False,
                        **kwargs):
        self.model, self.dls, self.loss_function, self.lr = model, dls, loss_function, lr
        ...
        self.num_patch = (max(args.context_points, args.patch_len)-args.patch_len) // args.stride + 1    
        self.context_points = args.context_points
    ...
    def fit(self, n_epochs, lr=None, cbs=None, do_valid=True):
        ...
        try:
            for self.epoch in range(n_epochs):            
                self('before_epoch')  # emtpy metric/lists...    
    ...
    def epoch_train(self):
        ...
        self.all_batches('train') # TRAINING function
    ...
    def all_batches(self, type_):
        if self.cls:
            if self.ft:
                ...
                sample_data, _ = next(iter(self.dl))
                _, total_T, _ = sample_data.shape
                
                start = np.random.randint(0, total_T - self.context_points, len(self.dl))
                end = start + self.context_points
                start = torch.tensor(start)
                end = torch.tensor(end)
                
                # same ts/te inside batch
                for num, batch in enumerate(self.dl):     
                    X = batch[0][:,start[num]:end[num],:] # (64,512,1)
                    y = batch[1]  # (64)
                    self.iter, self.batch = num, [X,y]            
                    if type_ == 'train': self.batch_train()
                    elif type_ == 'valid': self.batch_validate()
                    elif type_ == 'predict': self.batch_predict()             
                    elif type_ == 'test': self.batch_test()
    ...
    def batch_train(self):
        self('before_batch_train') # to cuda
        self._do_batch_train()
        self('after_batch_train')  # to cuda

    def batch_validate(self):
        self('before_batch_valid') # to cuda
        self._do_batch_validate()
        self('after_batch_valid')  # to cuda
    
    def batch_predict(self):
        self('before_batch_predict') # to cuda
        self._do_batch_predict()
        self('after_batch_predict')  # to cuda

    def batch_test(self):
        self('before_batch_test') # to cuda
        self._do_batch_test()
        self('after_batch_test')  # to cuda
        
    def _do_batch_train(self):        
        # forward + get loss + backward + optimize    
        #===================================#
        # self.batch = (X,y)
        # ( y is not used in pretraining )
        
        #===================================#
        # [ PRETRAINING & FINE TUNING ]
        #-------------------------------#
        # Classification datasetf
        # --- X (64,512,1)
        # --- y (64) 
        #-------------------------------#
        # Forecasting dataseet
        # --- X (64,512,1)
        # --- y (64,96,7) 
        #===================================#
        ...
        if self.cls:
            y = y.long().cuda()
        self.pred, self.loss = self.train_step([X, y])                                 
        #===================================#
        # [ PRETRAINING ]
        #-------------------------------#
        # Classification & Forecasting dataset
        # --- self.pred[0] =  [64, 32, 1(7), 12]
        # --- self.pred[1] =  [64, 32, 1(7), 12]
        #===================================#
        ...
        # --- self.pred[0] =  [64, num_class]
        # --- self.pred[1] =  [64, num_class]
        #-------------------------------#
        # Forecasting dataset
        # --- self.pred[0] =  [64, pred_len, C]
        # --- self.pred[1] =  [64, pred_len, C]
        #===================================#
    ...
    def train_step(self, batch):
        ...
        outs = self.model_forward()
        
        if self.ft:
            loss = self.loss_function(outs, self.yb)
            return outs, loss
        else:
            (pred1, pred2), loss_cl = outs
            #loss_mse = self.loss_func(outs, self.yb, loss_function=self.loss_function)
            loss_mse = self.loss_func(outs, self.yb)
            loss = loss_mse + loss_cl
    ...
    def valid_step(self, batch):
        ...
        if self.ft:
            loss = self.loss_func(outs, self.yb, loss_function=self.loss_function)
            return outs, loss
        else:
            (pred1, pred2), loss_cl = outs
            loss_mse = self.loss_func(outs, self.yb, loss_function=self.loss_function)
    ...
    def _do_batch_test(self):   
        ...
        if self.cls:
            y = y.long().cuda()
        self.pred, self.yb = self.test_step([X, y])  
    ...
    def lr_finder(self, start_lr=1e-7, end_lr=10, num_iter=100, 
                  step_mode='exp', show_plot=True, suggestion='valley'):                
        """
...
def transfer_weights2(weights_path, model, exclude_head=True, device='cpu'):
    # state_dict = model.state_dict()
    new_state_dict = torch.load(weights_path, map_location=device)
    #print('new_state_dict',new_state_dict)
    matched_layers = 0
