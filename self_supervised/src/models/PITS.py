...
class PITS(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, num_patch:int, 
                 ...
                 dropout = 0.1, max_len = 2000 , n_heads = 8, d_ff = 256, e_layers = 4,
                 num_experts = 4 , moe = False,
        ...
        # Backbone
        self.backbone = DeepMLPencoder(c_in = c_in, patch_len = patch_len,
                                        d_model= d_model, shared_embedding= shared_embedding,
                                        hidden_depth= hidden_depth, h_mode =h_mode, seq_mixing = seq_mixing,
                                        dropout = dropout, max_len = max_len, n_heads = n_heads, d_ff = d_ff, e_layers = e_layers,
                                        num_experts = num_experts , moe = moe,
                                        postional_embed = postional_embed, pool_size = pool_size,
                                        **kwargs)
