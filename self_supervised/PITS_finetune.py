...
parser.add_argument('--context_points_fn', type=int, default=512, help='sequence length fine-tune dataset')
...
def get_model(c_in, args, head_type, weight_path=None):
    ...
    """
    if args.ft:
        args.num_patches = (max(args.context_points_fn, args.patch_len)-args.patch_len) // args.stride + 1
...
def find_lr(head_type):
    ...
    cbs += [PatchCB(patch_len=args.patch_len, stride=args.stride, features_domain = args.feature_domain)]
...
def finetune_func(lr=args.lr):
    ...
    cbs += [
         PatchCB(patch_len=args.patch_len, stride=args.stride, features_domain = args.feature_domain),
