...
class PositionalEmbeddingXBiot(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 1000):
        super(PositionalEmbeddingXBiot, self).__init__()
...
class FilterBankEmbedding(nn.Module): # XBioT
    ...
    def forward(self, x):
        ...
        """
        x = self.projection(x)
        knorm = torch.linalg.norm(torch.exp(-torch.cdist(self.projection.weight, self.projection.weight)), dim = (0, -1), ord = 'fro').sum()
        return x, knorm
